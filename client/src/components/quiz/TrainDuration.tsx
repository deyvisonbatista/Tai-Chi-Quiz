...
const OPTIONS = [
  { text: "7 minutos", emoji: "⏰" },
  { text: "15 minutos", emoji: "⏰" },
  { text: "30 minutos", emoji: "⏰" },
  { text: "Vocês decidem", emoji: "🏆" }
];

export default function TrainDuration({ onNext, onBack }: Props) {
  return (
    <div className="min-h-screen bg-[#FDF6ED] flex flex-col items-center overflow-x-hidden font-sans relative pb-10">
      ...
      <main className="w-full max-w-[500px] flex flex-col items-center z-10 px-6">
        ...
        >
          Qual <span className="text-[#DE77A5]">duração</span> se encaixa melhor na sua rotina?
        </motion.h1>
        ...
        <div className="flex flex-col gap-[14px] w-full">
          {OPTIONS.map((option, index) => (
            <motion.button
              ...
              className="w-full bg-[#EBE0D6] hover:bg-[#E2D4C8] border border-[#E0D3C9] transition-all duration-300 rounded-[14px] py-[22px] px-6 flex items-center shadow-[0_2px_8px_rgba(0,0,0,0.03)] active:scale-[0.98] group"
            >
